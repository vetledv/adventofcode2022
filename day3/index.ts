...
const partTwo = () => {
  ...
  const resultArr = groups.map((group) => {
    ...
    const firstChars = stringToCharArray(first);
    const secondChars = stringToCharArray(second);
    const thirdChars = stringToCharArray(third);
    const [commonInAllThree] = firstChars.filter((char) => secondChars.includes(char) && thirdChars.includes(char));
